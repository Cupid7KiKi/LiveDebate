...
const SERVER_CONFIG = {
	...
	// 后端服务器地址（真实服务器，直接访问）
	BACKEND_URL: 'http://1.15.95.154:8000',
	// 当前使用的地址（修改这里切换服务器）
	get BASE_URL() {
...
async function controlStreamLive(streamId, start) {
	const streamName = window.liveSetupStreams?.find(s => s.id === streamId)?.name || streamId;
		...
		if (!confirm(start ? `确定要开始直播「${streamName}」吗？` : `确定要停止直播「${streamName}」吗？`)) {
		return;
	}
	...
	try {
		...
		if (typeof startLive === 'undefined' || typeof stopLive === 'undefined') {
			...
			if (typeof showToast === 'function') showToast('系统错误：API函数未加载', 'error');
			return;
		}
